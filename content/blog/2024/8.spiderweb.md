...
>:bulb: **Note**: Notice that we cast `EDGES` to `f64` - 64-bit floating-point literal to ensure that the division operation is a floating-point number. Rust is adamant about this and would complain otherwise.
...
>:bulb: **Note**: Rust also has an array type with a fixed size (known at compile time); e.g. `let array: [i8; 3] = [1, 2, 3]` is an array of length `3` and type `i8`.
...
>:bulb: **Reminder**: No need for `return` at the end of a function in Rust. The last value is implicitly returned.
...
>:bulb: **Note**: In Rust, external (i.e. non-native) libraries are called **crates**. There is a popular crate used for generating random numbers - [`rand`](https://docs.rs/rand/latest/rand).
