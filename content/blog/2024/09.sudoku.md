...
To fill in the remaining 54 cells, we can make use of a *Depth-first search* (DFS[^1]) algorithm. In essence, we randomly fill the remaining board, one cell at a time until all the rules are satisfied. For example, say we are trying to fill cell at position `(7, 8)` (in the form `(row, column)` with zero-based indexing[^2]) as seen in the following figure below. With the current state of the board, this cell has the following constraints (digits that cannot be placed in this cell):
