...
For the data generation part of this small project, I decided to get a bit out of my comfort zone and try to *Rust* it out 🦀
...
Note that *θ* is given in [radians](https://en.wikipedia.org/wiki/Radian). Meaning we need to convert from degrees to radians. That's another free ticket to high school: `radians = π / 180° × degrees`, remember? In Rust, it looks like this:

```rust [main.rs]
let radians: f64 = angle as f64 * std::f64::consts::PI / 180.0;
```

All that remains now is to generate the coordinates for the remaining points in the spiderweb, at varying "depths" (i.e. rings in the spiderweb), and to store it in the hash table we have defined earlier - `coordinates` (of type `BTreeMap`), using the angles at the origin as *keys*, and the list of subsequent coordinates on that strand as *values*.
